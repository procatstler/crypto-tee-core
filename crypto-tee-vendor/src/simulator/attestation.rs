...
impl AttestationService {
    ...
    pub fn new(config: AttestationConfig, device_identity: DeviceIdentity) -> VendorResult<Self> {
        ...
        let challenge_state = ChallengeState { challenges: HashMap::new(), challenge_counter: 0 };
    ...
    pub async fn generate_challenge(
        ...
        rng.fill(&mut nonce).map_err(|e| {
            VendorError::AttestationFailed(format!("Failed to generate nonce: {}", e))
        })?;
    ...
    pub async fn respond_to_challenge(
        ...
        // Get challenge
        let challenge = challenge_state.challenges.remove(challenge_id).ok_or_else(|| {
            VendorError::AttestationFailed(format!("Challenge not found: {}", challenge_id))
        })?;
...
impl CertificateAuthority {
        ...
        let signing_key = match config.attestation_algorithm {
            Algorithm::Ed25519 => {
                let pkcs8 = Ed25519KeyPair::generate_pkcs8(rng).map_err(|e| {
                    VendorError::KeyGeneration(format!("Ed25519 key generation failed: {}", e))
                })?;
                let key_pair = Ed25519KeyPair::from_pkcs8(pkcs8.as_ref()).map_err(|e| {
                    VendorError::KeyGeneration(format!("Ed25519 key parsing failed: {}", e))
                })?;
                CertificateSigningKey::Ed25519(key_pair)
            }
            Algorithm::EcdsaP256 => {
                ...
                .map_err(|e| {
                    VendorError::KeyGeneration(format!("ECDSA key parsing failed: {}", e))
                })?;
                CertificateSigningKey::EcdsaP256(key_pair)
            }
            _ => {
                return Err(VendorError::NotSupported(format!(
                    "Algorithm {:?} not supported for attestation",
                    config.attestation_algorithm
                )))
            }
        };
    ...
    fn generate_certificate(
        ...
        let certificate_data =
            format!("CERTIFICATE:{}:{}:{}:{:?}", serial_number, subject, issuer, cert_type)
                .into_bytes();

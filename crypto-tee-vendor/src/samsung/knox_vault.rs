...
use jni::{JNIEnv, objects::{JObject, JString, JValue}};
...
impl KnoxVault {
    ...
    pub fn is_available(env: &JNIEnv) -> VendorResult<bool> {
        ...
        let knox_vault_class = env.find_class("com/samsung/android/knox/keystore/KnoxSecurityUtils")
            .map_err(|_| VendorError::NotAvailable)?;
        
        let is_knox_vault_supported_method = env.get_static_method_id(
            knox_vault_class,
            "isKnoxVaultSupported",
            "()Z"
        ).map_err(|_| VendorError::NotAvailable)?;
    ...
    pub fn enable_for_key(env: &JNIEnv, spec_builder: JObject) -> VendorResult<()> {
        ...
        ).map_err(|e| VendorError::KeyGeneration(format!("Failed to find setKnoxVault: {}", e)))?;
    ...
    pub fn set_authentication(
        ...
        let set_user_auth_method = env.get_method_id(
            builder_class,
            "setUserAuthenticationRequired",
            "(Z)Lcom/samsung/android/knox/keystore/KnoxKeyGenParameterSpec$Builder;"
        ).map_err(|e| VendorError::KeyGeneration(format!("Failed to find setUserAuthenticationRequired: {}", e)))?;
        ...
        env.call_method_unchecked(
            ...
            jni::signature::ReturnType::Object,
            &[JValue::Bool(1)]
        ).map_err(|e| VendorError::KeyGeneration(format!("Failed to set user authentication: {}", e)))?;
        
        // Set validity duration if specified
        if let Some(seconds) = validity_seconds {
            let set_validity_method = env.get_method_id(
                builder_class,
                "setUserAuthenticationValidityDurationSeconds",
                "(I)Lcom/samsung/android/knox/keystore/KnoxKeyGenParameterSpec$Builder;"
            ).map_err(|e| VendorError::KeyGeneration(format!("Failed to find setUserAuthenticationValidityDurationSeconds: {}", e)))?;
            ...
            ).map_err(|e| VendorError::KeyGeneration(format!("Failed to set validity duration: {}", e)))?;
        }
    ...
    pub fn set_biometric_auth(env: &JNIEnv, spec_builder: JObject) -> VendorResult<()> {
        ...
        // Set biometric authentication
        let set_biometric_method = env.get_method_id(
            builder_class,
            "setUserAuthenticationValidWhileOnBody",
            "(Z)Lcom/samsung/android/knox/keystore/KnoxKeyGenParameterSpec$Builder;"
        ).map_err(|e| VendorError::KeyGeneration(format!("Failed to find setUserAuthenticationValidWhileOnBody: {}", e)))?;
    ...
    pub fn get_status(env: &JNIEnv) -> VendorResult<KnoxVaultStatus> {
        let knox_vault_class = env.find_class("com/samsung/android/knox/keystore/KnoxSecurityUtils")
            .map_err(|e| VendorError::HardwareError(format!("Failed to find KnoxSecurityUtils: {}", e)))?;
        ...
        let is_enabled_method = env.get_static_method_id(
            ...
            "isKnoxVaultEnabled",
            "()Z"
        ).map_err(|e| VendorError::HardwareError(format!("Failed to find isKnoxVaultEnabled: {}", e)))?;
        ...
        let enabled = env.call_static_method_unchecked(
            ...
            &[],
            jni::signature::ReturnType::Primitive(jni::signature::Primitive::Boolean)
        ).map_err(|e| VendorError::HardwareError(format!("Failed to check Knox Vault enabled: {}", e)))?;

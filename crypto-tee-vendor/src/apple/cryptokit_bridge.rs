...
impl CryptoKitOperations {
    ...
    pub fn generate_key(algorithm: Algorithm) -> VendorResult<CryptoKitKey> {
        match algorithm {
            ...
            _ => Err(VendorError::NotSupported(
                format!("Algorithm {:?} not supported by CryptoKit", algorithm)
            )),
        }
    }
    ...
    fn generate_p256_key() -> VendorResult<CryptoKitKey> {
        unsafe {
            ...
            let private_key_class = objc::runtime::Class::get("PrivateKey")
                .ok_or_else(|| VendorError::NotSupported("PrivateKey class not found".to_string()))?;
    ...
    fn generate_ed25519_key() -> VendorResult<CryptoKitKey> {
        unsafe {
            ...
            if key.is_null() {
                return Err(VendorError::KeyGeneration("Failed to generate Ed25519 key".to_string()));
            }
    ...
    pub fn verify(
        ...
        unsafe {
            ...
            let public_key_class = objc::runtime::Class::get(key_class_name)
                .ok_or_else(|| VendorError::NotSupported("PublicKey class not found".to_string()))?;
            ...
            if public_key_obj.is_null() {
                return Err(VendorError::VerificationError("Failed to create public key".to_string()));
            }

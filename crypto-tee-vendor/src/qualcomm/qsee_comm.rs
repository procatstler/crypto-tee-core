...
impl QSEECommunicator {
    ...
    pub fn new() -> VendorResult<Self> {
        let mut comm = Self { handlers: Mutex::new(HashMap::new()), session: Mutex::new(None) };
    ...
    pub async fn establish_session(&self) -> VendorResult<u32> {
        ...
        *self.session.lock().unwrap() =
            Some(QSEESession { session_id, established_at: std::time::SystemTime::now() });
    ...
    pub async fn send_command(&self, command_id: u32, data: &[u8]) -> VendorResult<Vec<u8>> {
        ...
        if let Some(handler) = handlers.get(&command_id) {
            ...
            Err(VendorError::NotSupported(format!("Unknown command: 0x{:04x}", command_id)))
        }
    }
...
impl From<QSEEResponseCode> for VendorError {
    fn from(code: QSEEResponseCode) -> Self {
        match code {
            ...
            QSEEResponseCode::NotSupported => {
                VendorError::NotSupported("Operation not supported".to_string())
            }
            QSEEResponseCode::InvalidParameter => {
                VendorError::InvalidParameter("Invalid parameter".to_string())
            }
            QSEEResponseCode::OutOfMemory => {
                VendorError::HardwareError("Out of memory".to_string())
            }
            QSEEResponseCode::AccessDenied => {
                VendorError::PermissionDenied("Access denied".to_string())
            }
        }
    }
}
...
impl QSEEMessage {
    ...
    pub fn parse_response(data: &[u8]) -> VendorResult<(QSEEResponseCode, Vec<u8>)> {
        ...
        let response_data = if data.len() > 4 { data[4..].to_vec() } else { Vec::new() };

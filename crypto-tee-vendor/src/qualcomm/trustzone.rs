...
impl TrustZoneApp {
    ...
    pub fn new() -> VendorResult<Self> {
        Ok(Self {
            loaded_apps: Mutex::new(HashMap::new()),
        ...
        let app_id = match app_name {
            ...
            _ => {
                return Err(VendorError::NotSupported(
                    format!("Unknown TrustZone app: {}", app_name)
                ));
            }
        };
    ...
    pub async fn send_command(
        ...
        // Get app handle
        let apps = self.loaded_apps.lock().unwrap();
        let handle = apps.get(app_name)
            .ok_or_else(|| VendorError::NotSupported(
                format!("TrustZone app not loaded: {}", app_name)
            ))?;
        ...
        match command {
            ...
            _ => Err(VendorError::InvalidParameter(
...
impl SecureBuffer {
    ...
    pub fn write(&self, data: &[u8]) -> VendorResult<()> {
        if data.len() > self.size {
            return Err(VendorError::InvalidParameter(

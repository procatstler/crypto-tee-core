...
impl SecureChannel {
    ...
    async fn establish_session_key(&self) -> VendorResult<SealingKey<NonceGen>> {
        ...
        self.rng.fill(&mut key_bytes)
            .map_err(|_| VendorError::KeyGeneration("Failed to generate session key".to_string()))?;
    ...
    pub async fn send_message(&self, message: &[u8]) -> VendorResult<Vec<u8>> {
        ...
        if *self.state.lock().unwrap() != ChannelState::Established {
            return Err(VendorError::InvalidState(
                "Secure channel not established".to_string()
            ));
        }
        ...
        if let Some(key) = session_key.as_mut() {
            key.seal_in_place_append_tag(Aad::empty(), &mut encrypted)
                .map_err(|_| VendorError::EncryptionError("Failed to encrypt message".to_string()))?;
        } else {
    ...
    pub async fn receive_message(&self, encrypted: &[u8]) -> VendorResult<Vec<u8>> {
        ...
        if encrypted.len() < 16 {
            return Err(VendorError::DecryptionError(
...
impl SecureChannel {
    ...
    pub fn parse_message(&self, data: &[u8]) -> VendorResult<ProtocolMessage> {
        ...
        Ok(ProtocolMessage::ApplicationData {

...
impl FileAuditStorage {
    ...
    /// Index a single file
    async fn index_file(&self, path: &PathBuf, index: &mut Vec<AuditEventIndex>) -> CryptoTEEResult<()> {
        ...
        while reader.read_line(&mut line).await
            .map_err(|e| CryptoTEEError::IoError(e.to_string()))? > 0 {
    ...
    async fn load_event(&self, index: &AuditEventIndex) -> CryptoTEEResult<AuditEvent> {
        let file = fs::File::open(&index.file_path).await
            .map_err(|e| CryptoTEEError::IoError(e.to_string()))?;

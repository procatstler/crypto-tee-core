...
fn bench_signing(c: &mut Criterion) {
    ...
    let setup = rt.block_on(async {
        let crypto_tee =
            CryptoTEEBuilder::new().build().await.expect("Benchmark operation should succeed");
    ...
    for (alias, algorithm, message, signature) in &test_data {
        group.bench_with_input(
            ...
            |b, (alias, message, signature)| {
                b.to_async(&rt).iter(|| async {
                    crypto_tee
                        .verify(alias, message, signature, None)
                        .await
                        .expect("Benchmark operation should succeed")
                });
            },
    ...
    for size in data_sizes {
        ...
        group.bench_with_input(
            ...
            |b, test_data| {
                b.to_async(&rt).iter(|| async {
                    setup
                        .sign("size_bench_key", test_data, None)
                        .await
                        .expect("Benchmark operation should succeed")
                });
            },

...
use crypto_tee::{CryptoTEE, CryptoTEEBuilder, HealthStatus, HealthConfig};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Platform: {} v{}", tee_health.platform_info.platform_type, tee_health.platform_info.platform_version);
    ...
    for i in 1..=3 {
        ...
        println!(
            ...
            i,
            periodic_report.overall_status,
            periodic_report.check_duration_ms
        );
        
        // Show any status changes
        if periodic_report.overall_status != health_report.overall_status {
            println!("⚠️  Health status changed from {:?} to {:?}",

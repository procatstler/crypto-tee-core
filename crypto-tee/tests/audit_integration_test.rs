...
async fn test_audit_logging_generates_logs() {
    ...
    let crypto_tee = CryptoTEEBuilder::new()
        .build()
        .await
        .expect("Failed to create CryptoTEE instance");
    ...
    let _key_handle = crypto_tee
        .generate_key("test_key_audit", options)
        .await
    ...
    let _signature = crypto_tee
        .sign("test_key_audit", data, None)
        .await
        .expect("Failed to sign data");

    // Delete the key
    crypto_tee
        .delete_key("test_key_audit")
        .await

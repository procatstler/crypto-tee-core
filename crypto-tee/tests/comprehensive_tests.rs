...
use helpers::{test_keys::TestKeyConfigs, test_scenarios::TestScenarios, TestConfig, TestHelper};
...
async fn test_all_algorithms_comprehensive() {
    ...
    for algorithm in algorithms {
        ...
        let helper = TestHelper::with_config(config).await.expect("Failed to create test helper");
...
async fn test_concurrent_operations() {
    ...
    TestScenarios::test_concurrent_operations(&helper)
        .await
...
async fn test_error_handling() {
    ...
    TestScenarios::test_error_scenarios(&helper).await.expect("Error scenarios test failed");
}
...
async fn test_key_metadata_management() {
    ...
    TestScenarios::test_key_metadata(&helper).await.expect("Key metadata test failed");
}
...
async fn test_large_data_operations() {
    ...
    TestScenarios::test_large_data_signing(&helper).await.expect("Large data signing test failed");
}
...
async fn test_key_listing_operations() {
    ...
    TestScenarios::test_key_listing(&helper).await.expect("Key listing test failed");
}
...
async fn stress_test_operations() {
    ...
    TestScenarios::stress_test(&helper).await.expect("Stress test failed");
}
...
async fn test_different_algorithms_interoperability() {
    ...
    for (i, algorithm) in algorithms.iter().enumerate() {
        ...
        helper.crypto_tee.generate_key(&alias, options).await.expect("Failed to generate test key");
    ...
    for (alias, algorithm) in &keys {
        ...
        let signature = helper
            .crypto_tee
            .sign(alias, &test_data, None)
            .await
            .expect("Failed to sign with algorithm-specific key");

        let valid = helper
            .crypto_tee
            .verify(alias, &test_data, &signature, None)
            .await
    ...
    // Clean up
    for (alias, _) in &keys {
        helper.crypto_tee.delete_key(alias).await.expect("Failed to delete test key");
    }
}
...
async fn test_key_usage_permissions() {
    ...
    helper.crypto_tee.generate_key(alias, options).await.expect("Failed to generate sign-only key");

...
use crypto_tee::{Algorithm, CryptoTEE, CryptoTEEBuilder, KeyOptions, KeyUsage, PlatformConfig};

#[tokio::test]
async fn test_cryptotee_initialization() {
    let tee =
        CryptoTEEBuilder::new().build().await.expect("CryptoTEE should initialize successfully");
...
async fn test_key_lifecycle() {
    ...
    let key_handle =
        tee.generate_key("test-key", options).await.expect("Test operation should succeed");
...
async fn test_sign_verify() {
    ...
    let options = KeyOptions {
        algorithm: Algorithm::Ed25519,
        usage: KeyUsage { sign: true, verify: true, ..Default::default() },

...
use crypto_tee::{CryptoTEE, CryptoTEEBuilder, types::KeyOptions};
...
impl Rfc9421TestHelper {
    ...
    async fn setup_test_key(&self, alias: &str, algorithm: Algorithm) -> Result<(), Box<dyn std::error::Error>> {
...
async fn test_basic_request_signing() {
    let helper = Rfc9421TestHelper::new().await
        .expect("Failed to create test helper");

    let key_alias = "rfc9421_test_key";
    helper.setup_test_key(key_alias, Algorithm::Ed25519).await
        .expect("Failed to setup test key");
    ...
    match result {
        Ok(signature_output) => {
            ...
            // Test verification
            let verify_result = helper.adapter.verify_message(
                &message,
...
async fn test_algorithm_mapping() {
    ...
    assert_eq!(
        map_algorithm(Algorithm::Ed25519),
        Some(SignatureAlgorithm::Ed25519)
    );
    assert_eq!(
        map_algorithm(Algorithm::EcdsaP256),
        Some(SignatureAlgorithm::EcdsaP256Sha256)
    );
    assert_eq!(
        map_algorithm(Algorithm::EcdsaP384),
        Some(SignatureAlgorithm::EcdsaP384Sha384)
    );
    assert_eq!(
        map_algorithm(Algorithm::Rsa2048),
...
async fn test_multiple_signature_algorithms() {
    ...
    for (crypto_algo, sig_algo) in algorithms {
        ...
        helper.setup_test_key(&key_alias, crypto_algo).await
            .expect("Failed to setup test key");
        ...
        match result {
            ...
            Err(e) => {
                ...
                assert!(
                    ...
                    "Algorithm {:?} should have reasonable error: {}", sig_algo, error_msg
                );
            }
        }
    }
